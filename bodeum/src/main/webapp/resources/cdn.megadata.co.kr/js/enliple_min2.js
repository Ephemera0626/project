    ...
    EN.prototype = {
        ...
        sendRf: function() {
            ...
            if (drcUrlParam) {
                ...
                var domainId = hostnm.replace(/^www\.|^m\./.exec(hostnm), "");
                ...
                script.onload = function() {
                    ...
                    f_enliple_sns_mkt_func.pageView(config);
                }
        ...
        sendConv: function() {
            ...
            try {
                ...
                for (var i = 0; i < product_list.length; ++i) {
                    ...
                    products.push({
                        'id': returnValue3 + product.product_no,
        ...
        isChromeSSL: function() {
            try {
                ...
                if (userAgent == null ||
                    ...
                    userAgent.indexOf("whale") != -1 ||
                    userAgent.indexOf("swing") != -1 ||
                    userAgent.indexOf("trident") != -1 ||
                    userAgent.indexOf("opera") != -1) {
                    return true;
                } else {
                    var version = userAgent.substring(userAgent.indexOf('chrome')).split(/\s/)[0];
                    if (userAgent.indexOf("iPhone") > -1) {
                        version = userAgent.substring(userAgent.indexOf('crios')).split(/\s/)[0];
                    }
                    version = version.split('/')[1].split('.')[0];
                    // 프로토콜 체크 && 버전
                    if (protocol == 'http:' && version > 79) { // http인 경우만 처리 한다.
                        return false;
                    }
                }
            } catch (e) {
                // 오류가 나는 경우 기존 로직을 태운다.
                return true;
            }
            // 기존 로직
            return true;
        },
        newMobSSL: function(param) {
            var mobScript = document.createElement('iframe');
            var encData = btoa(encodeURIComponent(this.iFrameDataSet(param)));
            //console.log(encData);
            mobScript.src = 'https://www.mediacategory.com/mediaCategory/js/mobAdverSSL.html?q=' + encData;
            mobScript.style = 'display:none;height:0;width:0;border:0;border:none;visibility:hidden;';
            document.head.appendChild(mobScript);
        },
        iFrameDataSet: function(param) {
            var mobDomain = 'https://log.mediacategory.com';

            if (param.indexOf('rdType=RFSHOP') > -1) {
                mobDomain = mobDomain + "/servlet/rfshop?";
            } else if (param.indexOf('rdType=RF') > -1) {
                mobDomain = mobDomain + "/servlet/rf?";
            } else if (param.indexOf('rdType=CONV') > -1) {
                mobDomain = mobDomain + "/servlet/conv?";
            } else {
                mobDomain = "";
            }

            //
            // switch (this.getData('rdType')) {
            //     case 'RF' :
            //         mobDomain = mobDomain + "/servlet/rf?";
            //         break;
            //     case 'RFSHOP' :
            ...
            var data = {};
            data.url = mobDomain; /* landing url */
            data.sendType = 'DEFAULT'; /* AJAX, DEFAULT ... */
            data.sendTypeOption = null; /* sendType Option Value */
            data.method = 'GET'; /* method type GET, POST, PUT, DELETE */
            data.dataType = 'QUERY'; /* QUERY : queryString, JSON : JSON */
            data.data = param; /* queryString or jsonData */
            data.appendParam = ["s_au_id", "s_IP_info"]; /* append Param array */
            //      console.log(data);
            return JSON.stringify(data);
        },
        send: function(param) {
            if (!this.isChromeSSL()) { // false 인 경우만 처리 한다.
                this.newMobSSL(param);
            } else {
